...
trait TransformToLaws[F[_], G[_], E1, E2] extends HandleToLaws[F, G, E1] with RaiseLaws[G, E2] {
  ...
  def pureTransform[A](a: A, f: E1 => E2)(implicit
      AF: Applicative[F],
      AG: Applicative[G]
  ): IsEq[G[A]] =
    F.transform(AF.pure(a))(f) <-> AG.pure(a)

  def raiseTransform[A](e1: E1, f: E1 => E2)(implicit
      R: Raise[F, E1]
  ): IsEq[G[A]] =
...
object TransformToLaws {
  def apply[F[_], G[_], E1, E2](implicit
      ev: TransformTo[F, G, E1, E2]
  ): TransformToLaws[F, G, E1, E2] =
